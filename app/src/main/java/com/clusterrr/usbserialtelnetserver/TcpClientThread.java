...
public class TcpClientThread extends Thread {
    ...
    public TcpClientThread(UsbSerialTelnetService usbSerialTelnetService, TcpServerThread tcpServerThread, Socket socket, String mode) throws IOException {
    ...
    private void proceedTelnetBuffer() throws IOException {
    ...
    public void write(byte[] data, int offset, int len) throws IOException {
        if (mMode == "telnet") {
            writeTelnet(data, offset, len);
        } else {
            writeWebSocket(data, offset, len);
        }
    }

    private void writeWebSocket(byte[] data, int offset, int len) throws IOException {
        if (mDataOutputStream == null)
            return;
        // TODO: do something more performant than byte[] to hex string
        mWebSocketService.sendMessage(Utilities.byteArrayToHexString(data, offset, len));
    }

    private void writeTelnet(byte[] data, int offset, int len) throws IOException {
